...
import { AppError, ErrorCode, createError, PasAsyncResult, ok, err, pasOk, pasErr } from './error'
 ...
 */
export async function readFile(filePath: string): PasAsyncResult<string> {
  try {
 ...
 */
export async function writeFile(filePath: string, content: string): PasAsyncResult<string> {
  try {
    ...
    return err(
      createError(
        `Unexpected error writing file: ${filePath} - ${errorObj.message || 'Unknown error'}`,
        ErrorCode.IO_ERROR
      )
    )
  }
 ...
 */
export async function ensureDir(dirPath: string): PasAsyncResult<string> {
  try {
 ...
 */
export async function findFiles(globPattern: string, cwd: string): PasAsyncResult<string[]> {
  try {
    ...
    try {
      ...
      return err(
        createError(
          `Error finding files with pattern ${globPattern}: ${errorObj.message || 'Unknown error'}`,
          ErrorCode.IO_ERROR
        )
      )
    }
    ...
    return err(
      createError(
        `Error finding files with pattern ${globPattern}: ${error.message || 'Unknown error'}`,
        ErrorCode.IO_ERROR
      )
    )
  }
 ...
 */
export async function copyFile(source: string, destination: string): PasAsyncResult<string> {
  try {
    ...
    return err(
      createError(
        `Unexpected error copying file: ${source} to ${destination} - ${errorObj.message || 'Unknown error'}`,
        ErrorCode.IO_ERROR
      )
    )
  }
 ...
 */
export async function pathExists(pathToCheck: string): PasAsyncResult<boolean> {
  try {
    try {
    ...
    return err(
      createError(
        `Error checking if path exists: ${pathToCheck} - ${errorObj.message || 'Unknown error'}`,
        ErrorCode.IO_ERROR
      )
    )
  }
 ...
 */
export async function cleanDir(dirPath: string): PasAsyncResult<string[]> {
  try {
    try {
    ...
    try {
      ...
      return err(
        createError(`Error cleaning directory: ${dirPath} - ${errorObj.message || 'Unknown error'}`, ErrorCode.IO_ERROR)
      )
    }
  } catch (error) {
    const errorObj = error as { message?: string }
    return err(
      createError(
        `Unexpected error cleaning directory: ${dirPath} - ${errorObj.message || 'Unknown error'}`,
        ErrorCode.IO_ERROR
      )
    )
  }
