...
it('should create files and directories using the JSON structure', async () => {
  ...
  const testDir = await fileTestUtil.setup(TEST_MODULE, {
    'file.txt': 'content',
    'empty-dir': null,
    'nested': {
...
it('should handle complex nested structures', async () => {
  ...
  expect(JSON.parse(await fileTestUtil.readFile(TEST_MODULE, 'config.json'))).toEqual({ name: 'test', version: '1.0.0' });
  expect(await fileTestUtil.readFile(TEST_MODULE, 'src/index.js')).toBe('console.log("Hello world");');
  expect(await fileTestUtil.readFile(TEST_MODULE, 'src/components/Button.js')).toBe('export const Button = () => <button>Click me</button>;');
});
